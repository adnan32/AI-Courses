...
b=ones(1,11)/11; %bk koefficienter för LTI FIR system. I detta exemlep är det one sample difference
...
n=0:N-1;
A=2; %Amplitud
k=300;%ger systemets Babs(k+1)=1.0018 och Bfas(k+1)=1.0462 rad (kompensering för MATLABs vektor index börjar på 1)
fas=0; %Fas angiven i grader
s=A*sin((2*pi/N)*k*n+fas*(pi/180)); %ren signal

y=conv(s,b,'same');%faltnin av signalen med systemet
figure
plot(n(1:end-1)+4,y(1:end-1))%y(1:end-1) kan behövas skjusteras om fördröjd i förhållandet till s2(2:end) 
hold on
